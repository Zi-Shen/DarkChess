...
public class ChessGameFrame extends JFrame {
    ...
    private void addRankingList() {
        JButton rankinglist = new JButton("Ranking List");
        rankinglist.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 - 70 * 1);
        rankinglist.setSize(150, 30);
        rankinglist.setFont(new Font("Rockwell", Font.BOLD, 16));
        rankinglist.setForeground(Color.DARK_GRAY);
        rankinglist.setBackground(Color.WHITE);
        add(rankinglist);
        buttonList.add(rankinglist);
        rankinglist.addActionListener((e) -> {
            System.out.println("Click RankingList button");
            JFrame frame = new JFrame("RankingList");
            frame.setSize(300, 300);
            JPanel panel = new JPanel();
            frame.add(panel);
            RankingListtypography(panel);
            String[] title = {"用户名","胜利场次"};
            Object[][] RankingData = new Object[11][2];
            Object[][] RankingData2 = new Object[10][2];

            try {
                File file =new File("gamedata\\RankingData\\RankingData.txt");
                ...
                try {
                    while (((t = inputStreamReader.read()) != -1)) {
                        String str2 = String.valueOf((char) t);
                        str3 = new StringBuilder((str3.append(str2).toString()));
                    }
                    if (((t = inputStreamReader.read()) == -1)) {
                        String str4 = str3.toString();
                        String[] str5 = str4.split(" ");
                        int[] str6 = new int[str5.length];
                        Map<String, Long> countMap = Arrays.stream(str5).collect(Collectors.groupingBy(String::intern, Collectors.counting()));
                        List<Map.Entry<String, Long>> list = countMap.entrySet()
                                .stream()
                                .sorted((p2, p1) -> {
                                    if (Objects.equals(p1.getValue(), p2.getValue())) return p1.getKey().compareTo(p2.getKey());
                                    return p1.getValue().compareTo(p2.getValue());
                                }).toList();
                        int i =0;
                        for (Map.Entry<String, Long> m : list) {
                            i++;
                            RankingData[i][0] = m.getKey();
                            RankingData[i][1] = String.valueOf(m.getValue());
                        }
                        for (int j = 1; j < RankingData.length; j++) {
                            System.out.print(RankingData[j][0] + " " + RankingData[j][1] + "\n");
            ...
            for (int i = 0; i <RankingData.length-1 ; i++) {
                for (int j = 0; j <RankingData[i].length ; j++) {
                    RankingData2[i][j]=RankingData[i+1][j];
                }
            }
            JTable table = new JTable(RankingData2,title);
            JScrollPane pane=new JScrollPane(table);
            frame.add(pane);
            frame.setVisible(true);
        });
    }
    private void RankingListtypography(JPanel panel) {
    }
    ...
    private void addSaveButton() {
        ...
        saveButton.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 + 70 * 0);
    ...
    private void addLoadButton() {
        ...
        loadButton.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 + 70 * 1);
    ...
    private void geneNextStepButtons() {
        ...
        nextStepButton.setLocation(WIDTH * 3 / 5 + 120 + 40, HEIGHT / 10 + 170 + 70 * 3 - 5);
    ...
    private void geneLastStepButtons() {
        ...
        lastStepButton.setLocation(WIDTH * 3 / 5 - 30 - 20, HEIGHT / 10 + 170 + 70 * 3 - 5);
    ...
    private void addExitReplayButtons() {
        ...
        undoButton.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 + 70 * 3);
    ...
    private void addRestartButton() {
        ...
        restartButton.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 + 70 * 2);
    ...
    private void addCheatButton() {
        ...
        cheatButton.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 + 70 * 4);
    ...
    private void addThemeButton() {
        ...
        themeButton.setLocation(WIDTH * 3 / 5, HEIGHT / 10 + 170 + 70 * 5);
